namespace Day2
{
    internal class Part2
    {
        internal async Task Start()
        {
            ...
            Console.WriteLine($"Part B: The total score of opponent B: {totalScoreOpponentB}");
        ...
        Round ParseRound(ReadOnlySpan<char> line)
        {
            ...
            switch (inputOpponentB)
            {
                case "Y": // draw
                    round.ChoiceOpponentB = round.ChoiceOpponentA;
                    break;
                case "X": // we need to lose
                    if (round.ChoiceOpponentA == TurnChoice.Rock) round.ChoiceOpponentB = TurnChoice.Scissor;
                    if (round.ChoiceOpponentA == TurnChoice.Paper) round.ChoiceOpponentB = TurnChoice.Rock;
                    if (round.ChoiceOpponentA == TurnChoice.Scissor) round.ChoiceOpponentB = TurnChoice.Paper;
                    break;
                case "Z": // we need to win
                    if (round.ChoiceOpponentA == TurnChoice.Rock) round.ChoiceOpponentB = TurnChoice.Paper;
                    if (round.ChoiceOpponentA == TurnChoice.Paper) round.ChoiceOpponentB = TurnChoice.Scissor;
                    if (round.ChoiceOpponentA == TurnChoice.Scissor) round.ChoiceOpponentB = TurnChoice.Rock;
                    break;
            }
